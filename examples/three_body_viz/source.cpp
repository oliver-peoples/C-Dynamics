...
int main()
{
    ...
    for (long double dt : dts)
    {
        ...
        cgeo::projection::SimplePinholeCamera<long double> m1_cam ,m2_cam, m3_cam;

        hmath::Vector3<long double> camera_center = { 0,0,d_em };
        hmath::Quaternion<long double> orientation = { 0.707,0,0.707,0 };

        m2_cam.simpleSetup(camera_center, orientation, 300, 1280, 960, 320, 240);
        ...
        m1_csv.open("../three_body_viz/m1_" + std::to_string(dt) + "_.csv");
        m2_csv.open("../three_body_viz/m2_" + std::to_string(dt) + "_.csv");
        m3_csv.open("../three_body_viz/m3_" + std::to_string(dt) + "_.csv");
        ...
        while (t_0 < t_n)
        {
            ...
            m2_cam.camera_center = env.particles[1].entity_state.KD0();
            m2_cam.camera_center.k = d_em * 0.01;
            m2_cam.projectPoint(env.particles[0].entity_state.KD0(), false, 10, -1, { 0,0,255 });
            m2_cam.projectPoint(env.particles[1].entity_state.KD0(), true, 1);
            m2_cam.projectPoint(env.particles[2].entity_state.KD0(), true, 1, -1, { 255,0,0 });

            cv::imshow("Orbits", m2_cam.frame);
            cv::waitKey(1);
